...
class TestProgVarReplaceVisitor {
    ...
    void testFunction() throws IOException {
        ...
        StatementVariableDeclaration initial = (StatementVariableDeclaration) dstm.getDeclarations().getFirst();
        ...
        ProgVarReplaceVisitor replacer = new ProgVarReplaceVisitor(initial, map, false, services);
        replacer.start();
        SolidityProgramElement result = (StatementVariableDeclaration) replacer.result();
//        assertEquals(end, replacer.result()); // stmnt.equals(repl.result())
    }
}
