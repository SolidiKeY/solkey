...
import static org.key_project.solidity.logic.equality.RenamingProgramElementProperty.RENAMING_PROGRAM_ELEMENT_PROPERTY;
...
public class RenamingTermProperty implements Property<Term> {
    ...
    private boolean unifyHelp(Term t0, Term t1, ImmutableList<QuantifiableVariable> ownBoundVars,
        ...
        if (op0 instanceof SModality mod0 && op1 instanceof SModality mod1) {
            ...
            nat = handleSolidity(mod0.programBlock(), mod1.programBlock(), nat);
            if (nat == FAILED) {
                return false;
            }
    ...
    /// Checks whether the given [SolidityBlock]s are equal modulo renaming and returns the
    /// updated
    /// [NameAbstractionTable] or [#FAILED] if the [SolidityBlock]s are not equal.
    ///
    /// @param sb0 the first [SolidityBlock] to compare
    /// @param sb1 the second [SolidityBlock] to compare
    /// @param nat the [NameAbstractionTable] used for the comparison
    /// @return the updated [NameAbstractionTable] if the [SolidityBlock]s are equal modulo
    /// renaming or [#FAILED] if they are not
    private static @Nullable NameAbstractionTable handleSolidity(SolidityBlock sb0,
            SolidityBlock sb1,
            @Nullable NameAbstractionTable nat) {
        if (!sb0.isEmpty() || !sb1.isEmpty()) {
            nat = checkNat(nat);
            if (SolidityBlocksNotEqualModRenaming(sb0, sb1, nat)) {
                return FAILED;
            }
        }
        return nat;
    ...
    /// Returns true if the given [SolidityBlock]s are not equal modulo renaming.
    ///
    /// @param rb1 the first [SolidityBlock]
    /// @param rb2 the second [SolidityBlock]
    /// @param nat the [NameAbstractionTable] used for the comparison
    /// @return true if the given [SolidityBlock]s are NOT equal modulo renaming
    public static boolean SolidityBlocksNotEqualModRenaming(SolidityBlock rb1, SolidityBlock rb2,
            @Nullable NameAbstractionTable nat) {
        SolidityProgramElement pe1 = rb1.program();
        SolidityProgramElement pe2 = rb2.program();
    ...
    private int hashTermHelper(Term term, ImmutableList<QuantifiableVariable> nameAbstractionList,
        ...
        if (op instanceof QuantifiableVariable qv) {
            ...
            hashCode = 17 * hashCode + hashSolidityBlock(mod);
    ...
    /// Computes the hash code of a Solidity block modulo bound renaming.
    ///
    /// The hash code is computed based on the hash code of the program element of the Solidity
    /// block.
    ///
    /// @param mod the [SModality] to compute the hash code for
    /// @return the hash code
    private int hashSolidityBlock(SModality mod) {
        final SolidityBlock sb = mod.programBlock();
        if (!sb.isEmpty()) {
            final SolidityProgramElement rpe = sb.program();
            return rpe != null ? RENAMING_PROGRAM_ELEMENT_PROPERTY.hashCodeModThisProperty(rpe) : 0;
        }
        // if the Solidity block is empty, we do not add anything to the hash code
        return 0;
    }
