...
public final class PosInProgram {
    ...
    /// returns the SolidityProgramElement at the given position
    ///
    /// @param pos the PosInProgram
    /// @param prg the SolidityProgramElement we walk through
    /// @return the SolidityProgramElement at the given position
    /// @throws IndexOutOfBoundsException if position <code>pos</code> refers to a non-existent
    /// program element
    public static SolidityProgramElement getProgramAt(PosInProgram pos,
            SolidityProgramElement prg) {
        SolidityProgramElement result = prg;
        for (int i = 0; i < pos.depth; i++) {
            // method getChildAt throws an array index out of bound if
            // it.next refers to a non-existing child
            result = (SolidityProgramElement) result.getChild(pos.pos[i]);
        }
    ...
    public SolidityProgramElement getProgram(SolidityProgramElement pe) {
