...
public class TacletIndex {
    ...
    /// adds a new Taclet with instantiation information to this index. If rule instance is not
    /// known
    /// rule is not added
    ///
    /// @param taclet the Taclet and its instantiation info to be added
    public void add(SolTaclet taclet) {
    ...
    public void add(NoPosTacletApp tacletApp) {
        ...
        } else if (taclet instanceof SolAntecTaclet) {
            insertToMap(tacletApp, antecList);
        } else if (taclet instanceof SolSuccTaclet) {
            insertToMap(tacletApp, succList);
        } else if (taclet instanceof SolNoFindTaclet) {
    ...
    private void insertToMap(NoPosTacletApp tacletApp,
        ...
        Object indexObj = getIndexObj((SolFindTaclet) tacletApp.taclet());
    ...
    /// Filter the given list of taclet apps, and match their find parts at the given position of
    /// the
    /// sequent
    ...
    ///
    /// @param pos the PosOfOccurrence describing the formula for which to look for top level
    /// taclets
    /// @param services the Services object encapsulating information about the Solidity
    /// datastructures
    /// like (static)types etc.
    ...
    /// with the given term. It is assumed that the map (key -> value mapping) (1) contains keys
    /// with
    /// the top operator of its value, if no Solidity block is involved on top level of the value
    /// and no
    /// update is on top level (2) contains keys with the class of its top Solidity operator of its
    /// value's Solidity block, if a Solidity block is involved on the top level (3) contains keys
    /// with the
    /// special 'operators' PROGSVOP and DEFAULTSVOP if the top Solidity operator or top operator
    /// (resp.)
    ...
    /// prefix elements
    private ImmutableList<NoPosTacletApp> getProgramTacletList(
    ...
    public void remove(NoPosTacletApp tacletApp) {
        ...
        if (rule instanceof SolRewriteTaclet) {
            removeFromMap(tacletApp, rwList);
        } else if (rule instanceof SolAntecTaclet) {
            removeFromMap(tacletApp, antecList);
        } else if (rule instanceof SolSuccTaclet) {
            removeFromMap(tacletApp, succList);
        } else if (rule instanceof SolNoFindTaclet) {
    ...
    private void removeFromMap(NoPosTacletApp tacletApp,
            HashMap<Object, ImmutableList<NoPosTacletApp>> map) {
        Object op = getIndexObj((SolFindTaclet) tacletApp.taclet());
    ...
    /// Inner class to track the occurrences of prefix elements in Solidity blocks
    private static class PrefixOccurrences {
        ...
        public int occurred(SolidityProgramElement pe) {
