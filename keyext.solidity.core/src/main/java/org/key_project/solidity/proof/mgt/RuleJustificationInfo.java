...
public class RuleJustificationInfo {
    ...
    public @Nullable RuleJustification getJustification(RuleApp r, Services services) {
        RuleJustification just = getJustification(r.rule());
        if (just instanceof ComplexRuleJustification crj) {
            return crj.getSpecificJustification(r, services);
        } else {
            return just;
        }
    }
