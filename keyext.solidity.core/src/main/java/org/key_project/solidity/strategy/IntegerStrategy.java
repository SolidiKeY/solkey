...
import org.key_project.solidity.strategy.termgenerator.DividePolynomialsProjection;
...
public class IntegerStrategy extends AbstractFeatureStrategy implements ComponentStrategy {
    ...
    public IntegerStrategy(Proof proof, StrategyProperties strategyProperties) {
        super(proof);
        this.tf = new ArithTermFeatures(proof.getServices().getTheoryInfo().getIntLDT());
    ...
    private RuleSetDispatchFeature setupApprovalDispatcher() {
        final RuleSetDispatchFeature d = new RuleSetDispatchFeature();
        final IntLDT numbers = getServices().getTheoryInfo().getIntLDT();
    ...
    private void setupNewSymApproval(RuleSetDispatchFeature d, IntLDT numbers) {
    ...
    private void setupInEqSimp(RuleSetDispatchFeature d, IntLDT numbers) {
