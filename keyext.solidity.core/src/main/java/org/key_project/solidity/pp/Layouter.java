...
public class Layouter<M> {
    ...
    /// Constructs a newly allocated Layouter which will send output to the given [Backend] and
    ...
    /// Print a break. This will print <code>width</code> spaces if the line is _not_ broken
    /// at this point. If it _is_ broken, indentation is added to the current indentation
    ...
    /// This leads to a call of the [#mark(Object)] method of the backend, when the
    /// material preceding the call to <code>mark</code> has been printed to the backend, including
    /// any inserted line breaks and indentation. The [Object] argument to <code>mark</code> is
    ...
    /// Remove and return the token from the _bottom_ of the delimStack
    ...
    private static abstract class StreamToken<M> {
        /// Send this token to the Printer [#out].
