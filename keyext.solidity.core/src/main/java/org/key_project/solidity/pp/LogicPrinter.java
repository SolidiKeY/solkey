...
import static org.key_project.solidity.pp.PosTableLayouter.DEFAULT_LINE_WIDTH;
...
public class LogicPrinter {
    ...
    /// set line width is the maximum of [DEFAULT_LINE_WIDTH] and the given
    /// value
    ///
    ...
    public void printTaclet(Taclet taclet, SVInstantiations sv, boolean showWholeTaclet,
        ...
        if (declareSchemaVars) {
            TacletSchemaVariableCollector svCollector = new TacletSchemaVariableCollector();
            svCollector.visit(taclet);
        ...
        if (showWholeTaclet) {
            printFind(taclet);
            if (taclet instanceof SolRewriteTaclet rt) {
    ...
    protected void printRewriteAttributes(SolRewriteTaclet taclet) {
        final var applicationRestriction = taclet.applicationRestriction();
        if ((applicationRestriction
                .matches(ApplicationRestriction.SAME_UPDATE_LEVEL))) {
        ...
        if ((applicationRestriction
                .matches(ApplicationRestriction.ANTECEDENT_POLARITY))) {
            layouter.nl().print("\\antecedentPolarity");
        }
        if ((applicationRestriction
                .matches(ApplicationRestriction.SUCCEDENT_POLARITY))) {
    ...
    protected void printFind(Taclet taclet) {
        ...
        if (taclet instanceof SolSuccTaclet) {
            printSequentArrow();
            layouter.brk();
        }
        ...
        printTerm(ft.find());
        layouter.end();
        if (taclet instanceof SolAntecTaclet) {
            layouter.brk(1);
            printSequentArrow();
        }
        layouter.brk(0, -2).print(")").end();
    ...
    protected void printSchemaVariable(SchemaVariable sv) {
        ...
        if (o == null) {
            ...
            if (o instanceof Term) {
                printTerm((Term) o);
            } else if (o instanceof SolidityProgramElement pe) {
    ...
    public void printFunctionTerm(Term t) {
        ...
        if (services != null) {
            ...
            BoolLDT bool = services.getTheoryInfo().getBoolLDT();
    ...
    /// </pre>
    ///
    /// The subterms are printed using [#printTermContinuingBlock(Term)].
    ///
    ...
    /// [#printTermContinuingBlock(Term)] for the idea. Otherwise, like
    /// [#printInfixTerm(Term,int,String,Term,Term,int)].
    ///
    /// @param l the left subterm
    /// @param assLeft associativity for left subterm
    /// @param name the infix operator
    ...
    /// @param v the [QuantifiableVariable] to be substituted
    ...
    public void printModalityTerm(String left, SolidityBlock jb, String right, Term phi, int ass) {
        assert jb != null;
        assert jb.program() != null;
        if (phi.op() instanceof SModality mod && mod.kind() instanceof ModalOperatorSV) {
            Object o = getInstantiations().getInstantiation(mod.kind());
            if (o instanceof SModality.SolidityModalityKind kind) {
    ...
    /// [#printTermContinuingBlock(Term)]. This currently only makes a difference for infix
    /// operators.
    ///
    ...
    private void printSourceElement(SolidityProgramElement element) {
    ...
    /// LogicPrinter sp)
    public void printProgramElement(SolidityProgramElement pe) {
