...
# Run the Solidity CLI on a specific .key problem in the examples directory
...
**Whenever the user asks you to "run", "look at", "verify", or "test" a `.key` file, always use the `solidityCli` Gradle task:**

```bash
./gradlew :keyext.solidity.core:solidityCli --args="<filename>.key"
```

`keyext.solidity.core/build.gradle` defines the `solidityCli` Gradle `JavaExec` task for running `org.key_project.solidity.CLI`. The task's working directory is `keyext.solidity.core/src/test/resources/org/key_project/solidity/examples` and its configured default argument is `problem2.key`, so `./gradlew :keyext.solidity.core:solidityCli` verifies that default example.

Gradle's `--args` is not an extra argument appended to the existing `args "problem2.key"` line; it replaces the JavaExec task arguments. To pass Solidity CLI flags, put the entire Solidity CLI argument list inside the single Gradle `--args` value:
